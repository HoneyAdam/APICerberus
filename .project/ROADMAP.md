...
- [x] **Multi-database support** — Implemented: `DB` interface in `internal/store/db.go` with `SQLiteDB` and `PostgresDB` implementations. `Store.Open()` branches on `store.driver` ("sqlite" or "postgres"). PostgreSQL uses pgx stdlib driver with connection pooling. Dialect-aware migrations (SQLite FTS5 vs PostgreSQL tsvector). `buildAuditWhere()` uses dialect-specific FTS. `ListWebhooksByEvent()` uses `json_array_elements` (Postgres) vs `json_each` (SQLite). All 244+ store tests pass.
...
- [x] **Documentation site** — Added `docs/README.md` (landing page with feature overview), `docs/quick-start.md` (5-minute tutorial), `docs/installation.md` (all installation methods), `docs/configuration.md` (complete config reference), `docs/rate-limiting.md` (all algorithms and Redis support). Existing `docs/` contains architecture, production, API docs, and migration guides.
